...
| 2026-09-18 | `MSG-20260918-011`: Partial deopt v0.3 — DependencyIndex (the runtime inverse map) implemented (`compiler/pipeline/src/DependencyIndex.cpp` + `include/b2/pipeline/DependencyIndex.h`). The inverse map: `DependencyId → set<(NodeId, RegionId, MethodId)>`. `record(dep, node, region, method)` stores the association (deduplicated; sorted by `(dep, node, region)` for deterministic iteration + binary-search dup detection). `invalidate(dep)` returns a sorted, deduplicated `DirtySet` (the nodes + regions that depend on `dep`; feeds into `expandDirtyClosure()` from v0.1 + `checkRegionSafety()` from v0.2). `retireMethod(method)` removes all associations for a method. `size()` + `distinctDependencies()` telemetry. 12 new unit tests in `tests/pipeline/DependencyIndexTests.cpp` (empty index, record + invalidate, multiple associations, dedup, sorted output, unknown dep, retireMethod, distinct deps, determinism, the speculative-devirtualization scenario where a ClassHierarchy dependency fires when a new subclass is loaded → invalidate() returns the Guard node + its region, invalid dep ignored, no-node-no-region ignored). 38/38 pipeline tests pass (was 26; the 12 new DependencyIndex tests are the 27th through 38th). Build + ctest verified 19/19 T1 corpus + 16/19 T2 differential + 12/12 ctest targets pass; ASan+UBSan clean. No behavior change: the v0.3 is NOT yet wired to the inline pass (the inline pass creates ClassHierarchy dependencies via `Graph::addDependency()` but does not call `DependencyIndex::record()` — the wiring is the v0 → v1 transition's work). The speculative-devirtualization survey: the GuardInline machinery (ICDG Phase 2) is fully implemented + tested (20+ `il_guard_*` tests in `tests/passes/InlineTests.cpp`); the gap is the runtime invalidation path (the DependencyIndex, now v0.3-implemented) + the wiring to the inline pass (open work). |
